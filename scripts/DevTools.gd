...
func create_label(lname, config=null):
...
"""Table Graph Sequence
	...
	When the user successfully inputs the key string, an event of _"key"_ is dispatched.
...
func _unhandled_input(event):
	"""Call stack_key for every valid keypress"""
...
func stack_key(event):
	"""Given an event, extract the char string and perform a drop or start
	sequence as required.
	"""
	#var str_code = OS.get_scancode_string(event.scancode).to_lower()
	var str_code = OS.get_keycode_string( event.keycode ).to_lower()
...
func fail_key(key, str_code, event):
	"""A sequence had been started, but this entry does not match the expected _next_
	for the sequence - as such is a fail key."""
	# api handler for the 'start' of a key sequence.
	#print('Failed: ', key)
	return
